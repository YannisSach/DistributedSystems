...
public class Simulation {
	...
	public static void Test (int ServerType,int k ) throws IOException{
		...
		randPort = ports.get(Util.getRandom(0, ports.size()-1));
		MySocket.send(randPort, "INIT,DEPART," + Util.hash("5") );
		...
		ports.add(Util.getPort());
		Server s = Util.getServer(ServerType, 11, 11-1,ports,k);
		s.start();
		MySocket.send(ports.get(0), "INIT,JOIN,"+Util.hash(""+11)+","+ports.get(11-1));
		...
		randPort = ports.get(Util.getRandom(0,1));
	    MySocket.send(randPort, "INIT,,You don't match I tell you,231");
